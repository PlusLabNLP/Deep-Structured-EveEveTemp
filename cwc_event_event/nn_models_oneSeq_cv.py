...
class BiLSTM(nn.Module):
    ...
    def forward(self, labels, sent, lidx_start, lidx_end, ridx_start, ridx_end, pred_inds=[], flip = False, causal = False, vat=False):
        ...
        '''
        if not self.training and not self.out_win_pred:
            out_win_pred = torch.zeros(self.num_classes)
            out_win_pred[self.none_idx] = 1.0
            assert len(pred_inds) == prob.size()[0]
...
class NNClassifier(REDEveEveRelModel):
    ...
    def create_features_tbd(self, ex, args):
    ...
    def parallel(self, ex, args):
        ...
        # matres dataset is based on tbd data
            ...
            return ex.doc_id, ex.id, self._label_to_id[ex.rel_type], self.create_features_tbd(ex, args)
    ...
    def predict(self, model, data, args):
        ...
        for data_id, label, (sent, pos, new_fts, rev, lidx_start, lidx_end, ridx_start, ridx_end, pred_ind) in data:
            ...
            if count % eval_batch == 0 or count == len(data):
                ...
                out, prob = model(labels, sents, lidx_start, lidx_end, ridx_start, ridx_end, pred_inds, flip=rev, causal=is_causal)
...
class REDEvaluator:
    ...
    def evaluate(self, test_data: Iterator[FlatRelation], args):
        ...
        with mp.Pool(processes=2) as pool:
            data = pool.map(partial(self.model.parallel, args = args), test_data)

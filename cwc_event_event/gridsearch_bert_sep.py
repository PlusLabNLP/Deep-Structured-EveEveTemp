...
if __name__ == '__main__':
    ...
    assert args_local.fold < 5
    if args_local.data_type != 'tbd':
        if args_local.fold == 0:
            args_local.params = {
                'hid': [30, 40, 50],
                'num_layers': [1, 2],
    ...
    param_perf = []
    param_perf_local = []
    pickle_path = 'best_param/bert/gridsearch_{}_uf{}_trainpos{}_joint{}_TrainOn{}_TestOn{}'.\
    ...
    pickle_path_local = 'best_param/bert/grid_local_{}_uf{}_trainpos{}_joint{}_TrainOn{}_TestOn{}'.\
        format(args_local.data_type, args_local.usefeature, args_local.train_pos_emb, 
               args_local.joint, args_local.trainon, args_local.teston)
    ...
    for param_local in ParameterGrid(args_local.params):
        ...
        if path.exists("../ILP/bert/"+str(save_name)+".pt"):
        ...
        if args_local.data_type =='matres':
            if test_f1_local < 0.77:
                continue
        if args_local.data_type =='new':
            if test_f1_local < 0.765:
                continue
        if args_local.data_type =='tbd':
            if test_f1_local < 0.615:
                continue
